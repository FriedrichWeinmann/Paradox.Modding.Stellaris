Describe "Validating the module manifest" {
	$moduleRoot = (Resolve-Path "$global:testroot\..\Paradox.Modding.Stellaris").Path
	$manifest = ((Get-Content "$moduleRoot\Paradox.Modding.Stellaris.psd1") -join "`n") | Invoke-Expression
